...
class ScientificResult:
    ...
    #: Set by __post_init__ when the adapter's stated status and the derived
    #: status disagree. None means they agreed, or the adapter passed the base.
    status_expectation_mismatch: str | None = field(default=None, compare=False)
    ...
    def __post_init__(self) -> None:
        ...
        derived = derive_publication_status(base, coerced)
        # The adapter's own `publication_status` argument is an *expectation*.
        # Previously it was silently discarded, which meant an adapter could
        # drift from its own stated intent with nothing to notice: writing
        # publication_status="publishable" and receiving "qualified" produced no
        # signal at all. Silent agreement and silent disagreement looked
        # identical.
        #
        # It is now verified. Where the expectation and the derivation differ,
        # the derivation wins and the disagreement is recorded on the result, so
        # a drifting adapter is visible rather than quietly corrected.
        asserted = self.publication_status
        object.__setattr__(self, "publication_status", derived)
        if asserted != derived and asserted != base:
            object.__setattr__(
                self, "status_expectation_mismatch",
                f"adapter expected {asserted!r}; the algebra derived {derived!r} "
                f"from base {base!r} and {len(coerced)} qualification(s)")
        else:
            object.__setattr__(self, "status_expectation_mismatch", None)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "status_expectation_mismatch": self.status_expectation_mismatch,
